...
balances_225 %>% writexl::write_xlsx("balances_225.xlsx")
```
...
spend_225 <-
  ...
  mutate(across(spend_type, \(x) str_remove(x, "_ltd$"))) %>%
  mutate(across(spend_type, \(x) str_to_sentence(x)))

spend_225 %>% writexl::write_xlsx("spend_225.xlsx")
```
